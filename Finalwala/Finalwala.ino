...
void loop() {
  ...
  else if (angle >= 6 && appliedforce > ThresholdForce && rpm < 10 && clutchsensorState == LOW) {
...
void speed() {
  ...
  if (currentTime - lastTime >= 500) {   // every .5 second calculation of speed
    ...
    velocity = rpm * 3 * PI * Radius / 25; // check garna baki xa

    // lcd.setCursor(0, 0);
    // lcd.print("RPM:"); lcd.print(rpm, 0); lcd.print("    "); 
    // lcd.setCursor(0, 1);
    // lcd.print("Vel:"); lcd.print(velocity, 0); lcd.print(" km/h  ");
